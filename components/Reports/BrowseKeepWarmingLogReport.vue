     ...
     <p class="list-bar__sub-heading">{{ingredient.patch_number}}</p>
        ...
        <div class="list-bar list-bar__report">
          ...
          <div class="list-bar__text-container">
            ...
            <div v-for="(readings,index) in report.warmkeeping_log" class="report__info">
              <h6 class="report__sub-title">Next Reading</h6>
              <p class="report__info">{{readings.logged_data}} &deg; taken at: {{formatTimeStampTime(readings.logged_at)}}</p>
              <br>
            </div>
...
methods: {
  ...
  closeModal() {
    ...
    async showExceptionModal(){
      ...
      this.mixpanelTrack('Click: Repors Page - Browse Keep Warming Log Report Modal');
