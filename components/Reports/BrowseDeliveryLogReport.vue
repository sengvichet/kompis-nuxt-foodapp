<template>
  ...
  <div v-if="!isLoading" class="exception-internal">
   ...
   <div v-if="true" class="list-bar__log-item">
     <img class="list-bar__log-item--image" :src="report.logData.ingredients[0].image_url" />
   </div>
   ...
   <div class="list-bar__text-container"> <!-- this keeps the text in a flex-column with text-lign: left, to format text, and keep the icons to the right-->
     <h5 class="list-bar__title">{{report.logData.senderName}}</h5>
     <p class="list-bar__sub-heading">{{report.logData.ingredients[0].patch_number}}</p>
   </div>
        ...
        <div class="list-bar list-bar__report">
          ...
          <div class="list-bar__text-container">
            ...
            <div v-if="report.logData.logStatus">
              <h6 class="report__sub-title">Food Condition</h6>
              <p class="report__info">{{report.logData.logStatus}}</p>
              <br>
            </div>

            <div v-if="report.logData.typeOfGoods">
              <h6 class="report__sub-title">Food Type</h6>
              <p class="report__info">{{report.logData.typeOfGoods}}</p>
              <br>
            </div>

            <div v-if="report.logData.firstReading && report.logData.logStatus !='bad'" class="report__info">
...
methods: {
  ...
  closeModal() {
    ...
    async showExceptionModal(){
      ...
      this.mixpanelTrack('Click: Repors Page - Browse Delivery Log Report Modal');
